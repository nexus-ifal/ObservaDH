...
const PageContent: React.FC = () => {
  ...
  const { profissoes, isLoadingProfissoes, error: errorProfissao } = useProfissao();
  const { ideologiaGenero, isLoadingIdeologiaGenero, error: errorIdeologiaGenero } = useIdeologiaGenero();
  const { religiaoRaca, isLoadingReligiaoRaca, error: errorReligiaoRaca } = useReligiaoRaca();

  const { politicosFiltrados, isLoadingPoliticosFiltrados, error: errorPoliticoFiltrados } = usePoliticoFiltrados({
  ...
  const dropdownItems = useMemo(
    () => [
      ...
      {
        elementos: estados?.map((estado) => ({ titulo: estado.nome, value: estado.id })) ?? [],
        titulo: "Estado",
        param: "estadoId",
      },
      { elementos: generos, titulo: "Gênero", param: "generoId" },
      {
        elementos: partidos?.map((partido) => ({ titulo: partido.nome, value: partido.id })) ?? [],
        titulo: "Partidos",
        param: "partidoId",
      },
      {
        elementos: profissoes?.map((profissao) => ({ titulo: profissao.nome, value: profissao.id })) ?? [],
        titulo: "Profissão",
        param: "profissaoId",
      },
    ],
    ...
    <section className="flex flex-col gap-6 des:gap-10 justify-center">
      <header className="flex flex-row w-full px-2 des:px-16 h-14 des:h-[4.25rem] bg-[#122144] border border-b-0 border-white rounded-t-[5px] font-semibold text-[10px] tab:text-lg des:text-2xl text-[#87D9FF]">
        ...
        <section className="w-1/2 h-full px-2 des:px-12 grid grid-cols-3 gap-2 des:gap-4 items-center">
          <p>{"Partido"}</p>
          <p>{"Estado"}</p>
          <FiltrarPropostas handleClick={() => handleClick()} getSortIcon={() => getSortIcon()} />
        </section>
      </header>
      <section className="min-h-96 max-h-96 des:min-h-[400px] des:max-h-[800px] w-full rounded-md flex flex-col items-center gap-6 des:gap-10 overflow-auto">
        {isLoadingDados && Array.from({ length: 5 }).map((_, i) => <SkeletonParlamentar key={i} />)}
        
        {!isLoadingDados && dadosParlamentares.length === 0 && (
          <p className="text-white text-lg sm:text-2xl">{"sem resultados."}</p>
        )}
        
        {!isLoadingDados && dadosParlamentares.map((parlamentar, i) => (
  ...
  >
    ...
    <section className="flex flex-col gap-2 tab:gap-6 des:gap-10 justify-center">
      ...
      <section className="h-96 des:h-[800px] w-full rounded-md flex flex-col items-center gap-6 des:gap-10 overflow-auto">
        {isLoadingPartidos ? (
          Array.from({ length: 5 }).map((_, i) => <SkeletonPartido key={`skel-partido-${i}`} />)
        ) : (
          <>
            {partidosOrdenados?.length === 0 && (
              <p className="text-white text-lg sm:text-2xl">{"Nenhum partido encontrado."}</p>
            )}
            {partidosOrdenados.map((item) => (
              <Card.ComponentePartido key={`${item.nome}-${item.sigla}`} partido={item} />
            ))}
          </>
        )}
      </section>
      ...
      <section className="flex flex-col des:flex-row justify-center gap-4 tab:gap-10 des:gap-20">
        ...
        <Card.Legenda legenda={legendas[0]}>
          <Texto.Raiz className="text-3xl des:text-5xl w-full">
            ...
            <Texto.Linha>
              <Texto.Pequeno.Titillium className="text-[#D974FD]">{"x Gênero"}</Texto.Pequeno.Titillium>
            </Texto.Linha>
          </Texto.Raiz>
        </Card.Legenda>
      </section>
      ...
      <section className="flex flex-col-reverse des:flex-row justify-center gap-4 tab:gap-10 des:gap-20">
        ...
        <CardLegenda legenda={legendas[1]}>
          <Texto.Raiz className="text-3xl des:text-5xl w-full">
            ...
            <Texto.Linha>
              <Texto.Pequeno.Titillium className="text-[#FF977A]">{"x Raça"}</Texto.Pequeno.Titillium>
            </Texto.Linha>
          </Texto.Raiz>
        </CardLegenda>
        {isLoadingReligiaoRaca ? (
